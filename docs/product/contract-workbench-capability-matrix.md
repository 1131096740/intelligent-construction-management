...
| 证据 | 状态 |
| --- | --- |
| Controller 源码路由 | 已扫描 200 条 |
| Web API 请求 | 已扫描 157 条 |
| 实际 Nest route manifest | 已通过 `app.init()` 读取，共 604 条；源码缺运行时 0 条，运行时缺源码 0 条 |
...
| GET | `/business-parties/create-capability` | getBusinessPartyCreateCapability | apps/web-admin/src/pages/business-parties/BusinessPartyCreatePage.vue<br>apps/web-admin/src/pages/business-parties/BusinessPartyListPage.vue | matched | 保留 | 否 | — |
| GET | `/business-parties/creation-result` | — |  | backend_without_frontend | 补入口 | 否 | — |
...
| POST | `/projects/:param/close-profit/downstream-cost/attestations/contract` | — |  | backend_without_frontend | 补入口 | 否 | — |
