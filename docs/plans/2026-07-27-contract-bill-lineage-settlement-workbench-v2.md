...
状态：`[~]`（2026-07-27；已完成生效阶段守恒内核，人工映射写入口、主任确认和页面待完成）
...
- 待完成：真实双连接并发确认、浏览器交互和完整映射组拓扑验收。

当前验证：合同版本生效、映射服务与合同提交就绪检查 Jest 36/36 覆盖自动一对一兼容、拆分承接、金额一分差额阻断、跨单位缺换算依据阻断、经办人替换草稿、主任确认、撤销未确认映射、确认竞争 CAS 及无确认映射的提交阻断；API/Web typecheck、lint 通过。一次性本机 PostgreSQL 16 已以完整 89 条迁移和 seed 验证现有只读预检及清单双连接保存，容器自动清理；映射确认的真实双连接场景仍待加入。
