...
状态：`[x]`（2026-07-27；人工映射确认由 T16 承接完成）
...
实施结果（2026-07-27）：服务端新增内部 `version_replace` 模式。原 `__rowKey` 且单位一致的 Excel 行原位更新，从而保留物理行与 lineage；空行标识新增，未列出的既有行继续纳入删除及正式结算占用校验。预检返回逐行 `unchanged/added/removed/one_to_one/manual_review`，并保留来源行和导入行，清晰暴露自动一对一建议。单位变化一律标为 `manual_review`、写入 `pending` 映射状态并返回“需人工复核”预检错误，应用操作整批失败关闭，避免部分落库。普通成功预检不再写入不符合约束的 `legacy_replace` 映射状态。业务页面固定使用“导入新版清单”，只在无错误预检后允许确认原子应用并刷新权威工作台，不再暴露技术模式或将本地候选载入误当作正式应用。人工映射确认已由 T16 承接：只允许按相邻版本保存显式来源数量、期初数量、金额与换算依据，并经合同部主任确认后落账。差异/Excel API Jest 56/56、清单编辑器 Web Vitest 21/21、API/Web typecheck、lint 与 `git diff --check` 通过。
...
状态：`[x]`（2026-07-27；真实历史隔离库默认只读预检已完成，未运行回填）
...
完成标准：在隔离恢复库上可重复运行默认只读预检；回填二次执行数为 0 的验证仅在获得独立回填授权后执行，尚不代表生产已回填。
...
验证：工具 Jest 5/5（不猜测、人工确认范围、守恒阻断、写入门禁和幂等复跑）；API typecheck、lint 与 `git diff --check` 通过。2026-07-27 经授权读取最新生产自然备份 `jiangkong-20260727-030001.dump`，服务端和本机 SHA-256 校验一致、`pg_restore --list` 为 1235 项；仅在一次性 PostgreSQL 16 容器中恢复并应用候选的 91 条迁移后运行默认只读预检，报告为 `resolved=1/manualReview=7/blocking=0`、目标指纹 `9eb9c1739181be2a`、digest `7119c260a403227a30355fcd6c952f9c8ff04477232e4f6b0fede6e4788f8e92`。预检结束后容器、恢复库、本机备份副本和报告均已删除。未连接生产数据库，未执行生产迁移、回填或业务数据写入。

### T15 P1 集成候选与发布门禁

状态：`[x]`（2026-07-27；本地自动化与真实历史隔离只读门禁完成）
...
验证：全仓测试 Shared 137/137、Web 126 文件 1156/1156、API 199 套通过（5 套条件跳过）、4352/4368 断言通过（16 条条件跳过）；Shared/API/Web typecheck、lint、`check:business-errors`、`check:ui`、Prisma validate/generate、API/Web production build 与 `git diff --check` 通过。Chromium/WebKit：合同详情 2/2、结算签章/PDF/最终件 6/6、合同清单原子导入与 100+ 行编辑 8/8、业务网格 36/36。后续 T16-T18 已补齐受控拆分/合并、追溯调价和资金回收入口，并分别验证其权限、守恒及审计边界。

环境级门禁验证（2026-07-27）：经授权读取最新生产自然备份 `jiangkong-20260727-030001.dump`，先完成服务端与本机 SHA-256 校验及 1235 项 `pg_restore --list` 检查，再仅在一次性 PostgreSQL 16 容器恢复库应用 91 条候选迁移并运行默认只读预检。报告结果为 `resolved=1/manualReview=7/blocking=0`，目标指纹 `9eb9c1739181be2a`，digest `7119c260a403227a30355fcd6c952f9c8ff04477232e4f6b0fede6e4788f8e92`。7 项 `manualReview` 未被推断、映射或回填；需另行人工确认清单与独立回填授权。验证后容器、恢复库、本机备份副本和报告均已删除。未连接生产数据库，未执行生产迁移、回填或业务数据写入；推送和部署仍需分别授权。

补充验证（2026-07-27）：新增 `verify:contract-settlement-v2:local`，仅允许本机 Docker endpoint，使用随机端口和凭据启动一次性 PostgreSQL 16，完成 91 条迁移的空库部署、V2 只读预检报告及合同清单整表保存的双连接并发证据（恰一成功、另一条拒绝，审计失败回滚）。临时容器和目录自动清理，报告仅写入调用方指定路径。本次隔离报告摘要为 `resolved=0/manualReview=2/blocking=0`、digest `e611dc6c33093d0cd43e259eb8af9bc4f617ab99cf9a277272b5a7b9a33253f4`，证明空库路径与工具可执行；真实历史恢复副本的独立默认只读预检已由本节环境级门禁完成。
...
状态：`[x]`（2026-07-27；仅本地前向实现与自动化验证，未执行生产迁移、回填或业务数据写入）
...
实施进展（2026-07-27）：无限额框架不再简单跳过清单预计数量/金额校验。草稿文件生成、预览和正式提交共用同一行级校验：有金额上限合同仍对数量和金额硬拦截；无限额框架的实际超出只有在该合同清单行填写 `overageReason` 时才可继续。该说明随结构化结算行冻结，工作台将其作为“框架超量说明”列展示；读取合同清单时服务端返回 `capacityPolicy`，页面明确告知“可超预计，但说明将随明细冻结并在主任审批时显示”。正式结算详情会将该说明投影到明细账本，并在当前审批人可办理时显示逐项高亮复核提示。合同和结算归档资料统一收敛至 `JgAttachmentPanel` 的文件事实卡片，原有当前密码、短时链接和下载审计边界不变。新增按需加载 PDF.js 画布核对组件：经当前密码和用途确认后，为冻结结算单与乙方签章原件分别生成审计过的私有短时预览票据；桌面双栏、移动端切换、同步/独立页码缩放旋转、按冻结版重新对齐及按用户/草稿/修订/文件身份的本地查看进度恢复均已接通。最终动作明确为一次“核对通过并关联扫描件”，不新增逐页勾选；加载和画布渲染使用代次保护，快速翻页、缩放或重复打开不会让旧 PDF 异步结果覆盖当前画面。页数、方向、尺寸差异保持提示而非自动拒绝，未新增 OCR 或正文比对。API Jest 25/25、Web Vitest 31/31、API/Web typecheck、API/Web lint、Web `check:ui`、production build 与 Chromium/WebKit 签章回归 6/6 通过；未执行人工 UAT。
...
`T01-T19` 的前向实现、自动化、本机隔离验证及真实历史恢复副本的默认只读预检均已完成。历史报告有 7 项 `manualReview`、无 `blocking`；这些项必须由业务人工确认，且回填需要单独的清单和授权。推送、部署、生产迁移及任何回填仍需分别授权，不会自动执行。
