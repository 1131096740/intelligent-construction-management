...
- 待完成：合同工作台人工映射 UI、真实双连接并发确认与完整映射组拓扑验收。

当前验证：合同版本生效、映射服务与合同提交就绪检查 Jest 35/35 覆盖自动一对一兼容、拆分承接、金额一分差额阻断、跨单位缺换算依据阻断、经办人替换草稿、主任确认、撤销未确认映射及无确认映射的提交阻断；API typecheck、lint 通过。
