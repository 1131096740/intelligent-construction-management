...
- 待完成：实现人工映射 UI、分配数量/金额和单位换算依据；
- 待完成：合同草稿经办人保存/撤销未确认映射、合同部主任可串行化确认和审计；
- 待完成：确认前提交门禁、并发确认与映射组拓扑检查。

当前验证：合同版本生效 Jest 覆盖自动一对一兼容、拆分承接、金额一分差额阻断和跨单位缺换算依据阻断；API typecheck、lint 通过。
