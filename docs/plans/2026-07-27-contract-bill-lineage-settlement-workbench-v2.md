...
实施进展（2026-07-27，第四段）：草稿创建与更新已在同一事务中替换 `SettlementDraftLine`，兼容 JSON 仍双写；不完整但合法的草稿行以 `pending_source` 保存，避免把提交级校验提前误用于草稿。行表已写入来源、名称/单位、数量、单价或直接金额、原因、备注和顺序。冻结版结算单和提交事务都会按排序读取结构化行：前者以其生成/复核冻结业务事实，后者以其重算来源占用并生成正式结算；仅旧草稿没有结构化行时才回退兼容 JSON。签证/变更行现必须具备类别、发生日期、名称、说明、计价依据，并支持数量乘单价或直接金额；数量乘分币单价的结果逐行四舍五入到分。正式行会冻结来源类别、发生日期、说明、计价依据、关联原结算行和来源合同版本；手工负向调整缺少关联原结算行会阻断。聚焦 API Jest 78/78、Shared/API typecheck、API lint 与 `git diff --check` 通过。关联原结算行的跨合同归属校验、附件、前端独立录入区，以及 V2 草稿零 JSON 回退门禁仍未完成，故本任务未完成。
