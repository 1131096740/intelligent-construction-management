...
- [x] 针对 validator.js `MaxLength` 将 variation selector 组合字符与 Web code point 计数处理不一致的剩余差异，合同/结算/付款/项目支出四个自审 DTO 均改用项目已有 `IsMaxUnicodeTextLength`，保留 `ValidateIf` + `IsString` 及现有中文错误。`"❤️".repeat(250)` 为 500 code points 并通过，再多一个 code point 统一拒绝。
