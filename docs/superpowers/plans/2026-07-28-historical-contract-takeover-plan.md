...
独立修订、独立主管确认；按合同类型生成历史期初结算或直接付款容量和逐笔
历史实付事实；同时建立项目唯一挂靠企业、上游合同/结算/付款、挂靠拨款/
扣款及挂靠企业对下合同、结算、付款的持续接管账本。
...
4. 使合同版本和付款条款生效。
5. 结算类合同创建唯一 `sourceType=historical_takeover` 的历史期初结算，即使
   金额为 0；通用直接付款合同不伪造结算。
6. 把历史实付和 allocation 从 draft 改为 activated，凭证绑定不移动、不复制；
   结算类 allocation 指向期初结算，直接付款类指向合同容量。
7. 创建历史预付款或异常超付 balance account 及唯一 opening entry；固定金额
   直接付款超过合同有效金额也进入异常超付，无固定总价不因缺少总额误报。
8. 将结算类期初结算的历史已付缓存设置为正常结算 allocation 合计；直接付款
   类只维护合同累计实付投影。
9. 回写父 takeover 兼容累计字段、激活幂等键、可空期初结算 ID 和时间。
...
RED 必须覆盖结算类零结算、直接付款类无期初结算、固定金额和无固定总价、
零付款、多付款、多凭证、预付款、异常超付、重复激活、文件独占绑定和金额
守恒。
...
8. 通用直接付款历史合同不创建期初结算；固定金额按剩余容量付款，无固定
   总价按累计风险和项目资金门禁付款。
...
## Task 11：建立项目唯一挂靠企业和签约付款主体不变量

### Step 1：先写 schema 与服务 RED

- 一个项目同一时刻只能有一家挂靠企业；
- 业主主合同签约双方固定为业主与该挂靠企业；
- 每份对下合同冻结签约主体类型：`affiliate` 或 `our_company`；
- `affiliate` 合同只能登记挂靠企业付款，不能创建我方付款执行；
- `our_company` 合同只能创建我方付款，不能登记挂靠代付；
- 变更项目挂靠企业不得改写历史合同、结算、付款和文件主体快照；
- 缺少或冲突挂靠企业时所有新上游/挂靠接管动作失败关闭。

### Step 2：实现

使用前向结构、唯一约束和事务内主体校验；不要仅依赖前端下拉。已有项目先
输出只读映射报告，冲突进入人工清单，不按名称猜测企业。

---

## Task 12：重构业主主合同和上游结算为外部事实

### Step 1：先写行为 RED

- 合同人员上传已线下签完的业主主合同，合同主管确认；
- 不创建我方合同审批、用章或签署任务；
- 未登记主合同、或我方对下合同累计金额超出主合同有效金额时只生成风险；
- 董事长或总经理在我方对下合同最终审批时必须显式确认该风险；
- 上游结算由项目预算人员录入并执行独立确认；
- 不要求不存在的预算主管节点；
- 上游结算确认不创建我方审批实例；
- 外部正式文件冻结版本、摘要和确认人。
...
审计 `budget_director` 的 schema、权限、seed、审批、页面和历史数据引用；当前
业务写权限合并到项目预算人员。不能安全迁移的历史岗位仅作只读兼容，不继续
生成新任务。

---

## Task 13：记录业主付款、挂靠拨款和扣款

### Step 1：先写资金事实 RED

- 业主向挂靠企业付款逐笔记录金额和日期，但不增加我方可用资金；
- 挂靠企业向我方拨款增加我方项目可用资金；
- 挂靠拨款允许先到账、后关联或核对结算；
- 系统不再新建 `owner_direct_payment` 和泛化 `other` 收款；
- 管理费、税费、保证金、保险费和其他扣款独立记账；
- 纯到账差额先进入待核对，不自动生成扣款或成本；
- 同一上游付款和同一挂靠拨款不能重复计入收入、现金或经营看板。

### Step 2：先写确认规则 RED

- 有书面依据：财务人员或财务主管均可录入、确认；
- 口头通知：财务人员录入，财务主管确认；
- 财务主管本人录入时可以执行独立确认动作；
- 确认使用当前密码、冻结手写签名版本并写审计；
- 口头和书面事实在读模型中分开；
- 已确认事实只能追加更正或反向记录。

### Step 3：实现

为上游付款、我方到账、扣款和待核对差额使用不同业务类型及唯一幂等键；共享
文件 ACL，但不共享会导致资金混算的单一“项目收款”字段。

---

## Task 14：实现挂靠企业对下业务持续接管

**目标：** 外部完成合同、结算和付款，我方只形成可复核不可变账本。

### Step 1：先写 RED

- 挂靠合同登记后不生成我方合同审批、用章或归档确认任务；
- 挂靠结算登记后不生成我方结算审批；
- 挂靠付款登记后不生成我方付款审批或我方资金执行；
- 正常挂靠付款必须关联已确认合同和已确认结算；
- 正式合同预付款可以在结算前登记，但必须符合外部合同冻结的预付款约定；
- 合同、结算、付款对象和挂靠企业主体一致；
- 外部文件可以补充，口头通知必须明确标记；
- 已确认记录不可覆盖，更正追加 delta/reversal；
- 同一外部文件和同一外部付款不得跨项目或跨业务重复绑定。

### Step 2：实现分岗位录入确认

合同事实由合同岗位录入/确认，结算事实由项目预算岗位录入/确认，付款和资金
事实由财务岗位录入/确认；每个动作返回服务端 `availableActions`，不让页面
自行推断。外部确认不是我方审批，不创建 ApprovalInstance。

---

## Task 15：登记挂靠企业与我方线下合同

- 该合同已经线下签完，只做合同人员录入、合同主管确认和文件冻结；
- 它表达挂靠企业与我方之间的权利义务，不替代业主主合同，也不自动生成业主
  回款；
- 挂靠企业向我方拨款不以完成该合同结算为硬前提；
- 后续需要核对时关联结算、拨款和扣款，不覆盖银行到账事实；
- 不进入复杂的我方对下合同审批、用章和付款链。
...
新增测试必须证明：业主付款不增加我方余额、挂靠拨款只增加一次、挂靠扣款不
由差额自动生成、我方与挂靠付款主体不能互换、外部接管全链不创建我方审批。
