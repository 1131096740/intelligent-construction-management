...
已从生产备份恢复到 `jiangkong_restore_*`，完成生产已知 61 个迁移到候选 69 个迁移的隔离演练，并核对迁移数、表/索引、历史审批 JSON、金额、税务事实、文件引用和只读查询；M69 为 54 项引用清单、54 个统一触发器、5 个统一函数、0 个旧函数。transition preview 实际为 `itemCount=0`/`blockedCount=0`，因此 apply、幂等和漂移门禁对本候选为不适用，未人工造数据。只有未来 preview 产生非空 manifest 时，才先修复“生产历史 `super_admin` 为非 UUID、CLI 仅接受 UUID 操作人”的兼容阻断，再重跑 apply/幂等/漂移门禁并申请独立窗口 B 授权。隔离资源已清理，未连接或修改生产业务库。
...
已固定不可变运行候选 `74d5d2449ab9e4232f2625f2805c64b1686ff314`，全量门禁、精确 UAT 和六视口 76 张截图均绑定该 SHA。随后只允许形成包含最新主线记录、报告和机器证据的“证据/文档 HEAD”；必须证明它相对 `74d5d244…` 的运行时代码树为零差异，不得把仅文档 HEAD 冒充已重跑 UAT 的运行候选。未经用户同时针对当时证据/文档 HEAD 和运行候选 `74d5d244…` 明确批准，不得推送、部署、执行生产迁移或终止旧实例。

> **2026-07-19 发布前独立规格审计校正**：Task 4/5 补齐公司级 `contract_staff` 正式授岗，并将公司级可授岗范围与全项目可见范围分离，保留合同员独立项目授岗；Task 13 为合同、结算、付款的委托/转交动作日志冻结双方关系，并在审批单显示姓名、岗位和关系，不暴露内部 ID；Task 21 将历史接管和税务修订税率与共享域统一为最多 3 位小数，数量和含税单价仍为 2 位。三项均先补失败测试后实现，定向回归通过；新精确 SHA 全量门禁和发布证据按本 Task 重新生成。
