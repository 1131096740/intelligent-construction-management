# 全项目迁移、能力收口与旧运行时清理实施计划

> **执行要求：** 按任务顺序实施并逐项核验门禁。本计划含生产敏感步骤，连接生产、迁移、清理、推送或部署仍需用户再次明确授权。

**目标：** 在不长期双写、不猜测历史事实的前提下，把现有草稿、模板、
合同类型、资金和挂靠接管切换到新工作流，建立全项目前后端能力矩阵，按
零调用证据退出旧接口和原生小程序运行时；检查点、旧字段及生产数据物理
删除只在独立 Release C2/M3 获得授权后执行。
...
## Task 11：扩展能力矩阵到全项目路由、封装和按钮

### Step 1：生成三份 machine-readable manifest

1. Nest controller 业务路由及 method/path/权限；
2. Web API 封装及 method/path/真实消费者；
3. 页面可见动作及其 `availableActions` key/API 封装。

检查器必须失败关闭：

- 页面按钮指向不存在路由；
- 前端 API 封装只有单测、没有生产消费者；
- 同一路由有多个同义写封装；
- 后端业务路由未声明页面、内部任务、外部接管或退出分类；
- 页面绕过服务端 `availableActions` 自行按角色/状态放行。

生成报告写入 `docs/product/whole-site-capability-matrix.md`；生产调用证据只记录
脱敏计数、时间窗和报告摘要，不提交访问 token、IP 明细或业务敏感内容。

---

## Task 12：执行已确认的旧接口退出集合

### Step 1：先锁定零调用门禁

每一组删除分别要求：

1. 静态 controller/client/page 调用为零或已替换；
2. 构建后的 route manifest 无旧目标；
3. 生产只读日志在批准观察窗口内为零调用；
4. 正式历史读取、PDF、审计和接管台账不依赖旧写路径；
5. 负向测试证明旧路由返回不存在或明确退役，而不是悄悄转写两套数据。

### Step 2：按组删除

1. 旧合同草稿列表、旧软作废、旧恢复；
2. 草稿检查点写入/恢复代码；表和正式文件先只读，物理清理留给 Release C2；
3. 合同清单逐行新增、修改、删除、排序四套写接口；
4. 旧 `/me/workbench-summary`；
5. 旧合同生命周期和结算生命周期台账；
6. 前端重复 `createContractDraft`、`submitContractApproval` 封装，不删除新工作台
   仍使用的后端路由；
7. 旧直接 `POST /settlements` 和其封装；
8. 合同资料规则和文件版式 `revoke` route/service/wrapper；
9. `owner_direct_payment` 和泛化 `other` 项目收款新建路径；
10. 结算例外额度页面、接口、服务和新写模型。

每组使用独立提交和回退点，不把所有删除压成一个不可审查提交。

---

## Task 13：迁移模板、场景、合同类型和岗位

- 场景多映射只读预检：明确唯一生效资料规则、默认版式和冲突清单；
- 已有模板/版式/条款的多个 published 状态规范为一个当前有效版本，其余标记
  自动换版停用，不改历史引用；
- 旧 `generic_contract` 只读分类为通用结算、通用直接付款或人工待分类；
- 固定金额和无固定总价不能按金额是否为零自动猜测；
- `budget_director` 引用盘点后迁到项目预算人员或历史只读；
- 现有项目挂靠企业冲突不得自动选择；
- 所有 apply 绑定报告 SHA-256、数据库指纹和逐记录 revision。

冲突清单非零时可以发布只读兼容，但不得开放依赖新分类的写动作。

---

## Task 14：分阶段退役原生微信小程序

### Release M0：只读依赖盘点

- `apps/miniprogram` 路由、接口、上传下载、登录和环境配置；
- `/auth/wx-login` 的调用和生产日志；
- `wxOpenid` 的非空记录、唯一约束和外键；
- 与响应式 Web 共用的手机号登录、文件、签名二维码和备份恢复代码。

### Release M1：关闭新入口

- 不再发布小程序构建；
- `/auth/wx-login` 对新客户端明确退役；
- 保留观察期和响应式 Web 回退；
- 手机端登录、签名、上传下载和业务办理完成 WebKit/真机 UAT。

### Release M2：删除专属运行时

- 删除 `apps/miniprogram`、专属构建脚本、路由和未共享配置；
- 删除小程序专属 API 封装与测试；
- 不删除移动 Web、共享认证、私有文件或二维码会话。

### Release M3：另行授权的数据清理

只有生产备份、隔离恢复、零调用观察和用户独立授权后，才前向删除或收口
`wxOpenid` 等专属字段。M2 完成不能表述为 M3 已完成。

---

## Task 15：扩展后的最终生产验收

除 Task 10 外，至少使用真实岗位完成：

1. 业务场景自动确定六类合同、资料规则和默认版式；
2. 风险停用阻断草稿，自动换版要求升级或主管确认；
3. 电脑二维码、手机横屏签名和审批 PDF 冻结版本；
4. 零星材料付款先行和收货先行各一条；
5. 零星费用实付直接办结；
6. 固定金额直接付款硬上限和无固定总价累计风险；
7. 项目自有资金优先、垫资补足、实付后占用和人工终止；
8. 业主付款、挂靠拨款、挂靠扣款和待核对差额；
9. 挂靠企业外部合同、结算、付款只接管不审批；
10. 我方签约与挂靠签约主体/付款主体交叉负向测试；
11. 能力矩阵无未知路由、孤儿封装和不存在按钮；
12. 响应式 Web 手机链路通过，小程序运行时已按获授权阶段退出。

发布记录必须分别声明合同工作台 B/C1/C2、小程序 M1/M2/M3 和生产数据迁移
状态，禁止用一个“全部完成”掩盖尚未授权的物理清理。
