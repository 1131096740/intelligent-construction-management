...
| 失败/陈旧告警 | 独立 Node 检查器、root shell 通知/去重包装器和 systemd service/timer 已按获批 `48b5ec3f…` 安装；生产复跑 13/13 测试，首次手工 service 与 11:45:51 首次自然 timer 均健康，原 cron、应用运行时和 51 个迁移未变化 |
...
| 清理 | 隔离数据库、候选 checkout、恢复工具和输入临时目录已删除；应用生产发布后保持 `b857a426…`、51 个完成迁移和健康状态，监控安装未切换应用工作树 |

恢复证据绑定的是运行候选 `434c41a0511b0701fdc8f28e9466dfc959ef4f59`。其后的应用发布 `b857a426…` 与监控来源 `48b5ec3f…` 没有新增数据库迁移或修改备份生成逻辑，因此没有冒充对后续 SHA 做精确恢复；若数据库迁移或备份生成链路再次变化，必须重新评估恢复演练。

永久 03:00 cron 的首个自然收据和本手册 3.1 节的“备份陈旧/失败”监控安装已经通过。后续按日常运维观察 timer 收据；若需要向真实收件人做受控故障/恢复通知演练，应提前通知收件人并使用隔离状态目录，不得改动真实备份证据。生产 API CAM 策略的非当前版本 3 继续保留，未经用户授权不得删除或切换。
