...
| 采购版本 | 冻结供应商、采购原因、经办人、材料明细、合计和附件快照 |
| 材料明细 | 保存材料名称、规格型号、单位、数量、单价、金额和用途 |
...
前端展示金额合计，后端以整数分和适合数量精度的数值类型重新计算。客户端传入的合计只能作为显示值，不能作为最终账本依据。
...
实际采购必须与当前有效版本完全一致。供应商、规格型号、数量或单价发生任何变化，都必须：
...
- 所有金额持久化使用整数分和 `BIGINT`。
- API 金额使用规范十进制字符串。
- 数量使用满足材料小数精度的定点数，不能使用 JavaScript 浮点数作为账本计算依据。
- 前端计算只用于即时反馈，后端必须重算。
