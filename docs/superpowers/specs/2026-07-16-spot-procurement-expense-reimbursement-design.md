...
> 2026-07-18 变更提示：本文件的“费用与报销”设计继续有效；零星采购、零星付款、收货、票据、办结、PDF、工作台和验收规则已由 `docs/superpowers/specs/2026-07-18-spot-procurement-real-form-redesign.md` 变更。两者冲突时以 2026-07-18 设计为准。

日期：2026-07-16

状态：用户已于 2026-07-16 书面确认；五阶段设计及收货结算修订均已冻结，零星采购实施计划已完成，费用与报销实施计划后续单独制定
...
- 以最终合格收货数量计算实际采购成本，并闭环处理未付额度、多付款退款和同项目同供应商余额。
...
- 收货差异结算只处理最终合格收货与批准数量之间形成的资金差额，不改写原采购审批事实。
...
| 收货确认 | 每张采购单保存一次最终收货结果、确认人、委托关系、照片、水印文件、时间和说明 |
| 收货材料结果 | 按材料行保存合格实际收货、不合格、无偿附赠数量及差异说明 |
| 收货复核 | 保存物资主管复核结论、意见和时间 |
| 收货差异处理 | 保存少货差额、未付额度取消、多付款退款或转供应商余额的处理事实 |
| 供应商退款记录 | 保存实际退款金额、到账日期、收款证明和财务登记事实 |
| 供应商余额账户 | 按同一项目、同一供应商汇总可用、已预留和已使用余额 |
| 供应商余额分录 | 保存余额形成、预留、释放、执行抵扣和反向调整的不可变事实 |
...
零星采购以“采购根单 + 当前有效版本 + 最终实际采购成本”作为发票覆盖的唯一汇总口径，付款申请只保存这笔发票金额具体对应哪些付款的明细关系。付款层关系不能再次增加采购覆盖金额，避免同一发票在采购层和付款层被重复计算。报销以费用明细作为覆盖口径。
...
发票和无票金额不得重叠占用同一业务金额。未办结业务的分摊和确认需要调整时必须通过冲销或失效记录处理，不得直接改写已生效历史；零星采购正式办结后按第 6.10 节锁定，不再允许票据更正。

有效发票分摊、待复核或已确认的无票/票据异常存在时，不允许撤销其所依赖的收货复核。必须先解除分摊、退回申请或冲销已确认事实，再撤销收货复核，避免票据证据悬挂到失效收货修订。

采购申请原定有票、但真实付款后供应商最终无法开票时，不允许经办人直接改成无票。采购经办人必须发起“票据异常确认”，填写原因并上传证明，由财务主管确认后，异常金额才转为已确认无票。该动作不新增支出，不重复发起采购或付款审批，但必须保留原预计票据条件、异常申请、证明、确认人和确认时间。
...
除第 6.9 节明确规定的最终收货少货差额、退款和供应商余额处理外，真实付款后的供应商、规格、价格、票据条件等采购事实差错只能通过未来单独设计的异常更正、退货或反向流水处理，本设计不允许静默覆盖。
...
系统必须满足“本次申请结算金额 = 供应商余额抵扣金额 + 公司本次实际付款申请金额”。第一张草稿可以默认带入当前剩余可申请金额，并优先建议使用同项目同供应商可用余额，但经办人可在提交前改为较小金额。后续仍有未申请金额时，经办人可以继续创建新的付款申请。

### 6.5 付款申请与审批

同一个采购根单允许存在多张付款申请，用于分期或分次结算。所有有效付款申请的结算申请金额合计不得超过当前有效采购版本的金额合计。

付款草稿不占用采购可申请金额或供应商余额；付款申请提交审批时才在事务中同时占用采购可申请金额和本次供应商余额。付款申请被退回、驳回、撤回、作废或因采购版本变化失效时释放占用与余额预留。即使多个草稿同时存在，后提交者也必须按最新剩余金额和最新可用供应商余额重新校验。
...
同一个采购可以混合使用两种路径，但每一张付款申请的公司实际付款部分只能选择一种路径。公司实际付款申请金额为零、全部使用供应商余额抵扣时，不要求填写银行收款账户，但收款对象仍锁定为冻结供应商。
...
每张付款申请拥有独立审批实例和独立付款审批单 PDF。付款审批单必须分别展示结算申请金额、供应商余额抵扣金额和公司实际付款申请金额。

### 6.6 实际付款与付款凭证

审批通过不代表已经付款或已经抵扣供应商余额，只进入“已批准待执行”。

一张付款申请可以由一笔或多笔实际付款记录覆盖，但实际付款累计不得超过该付款申请批准的公司实际付款金额。只有实际付款金额和有效付款凭证在同一受控事务中登记成功后，系统才能增加已付金额并在付款审批单中显示已付事实。供应商余额抵扣必须按第 6.9 节另行执行，不能挤占或伪造实际付款金额。
...
一张采购单只建立一张最终收货确认单，不按批次、车辆或到货次数拆分。即使材料分多辆车到场，也只是在同一张收货确认单中上传多张照片。采购经办人或受托人按材料明细一次填写最终结果，整体提交一次，物资主管整体复核一次。
...
不合格材料不计入合格实际收货数量。供应商承诺补货时，收货确认保持草稿，待最终结果确定后再整体提交；确认不再补货时，按少货差异处理。供应商无偿多送的材料记录为“无偿附赠数量”，不增加原采购数量、采购金额、付款金额或票据覆盖金额；供应商后续要求收费时必须另行发起采购。

收货确认至少上传一张材料或卸货现场照片。可以使用系统拍照，也可以从手机相册上传；系统不获取定位、不校验拍摄地点，也不以照片来源限制提交、复核或办结。照片来源只按“系统拍照”或“相册上传”留档。

所有正式收货照片均由服务端生成统一水印。水印采用照片下方扩展信息卡的样式，不在原照片主体上反复覆盖，固定字段至少包含：
...
水印字段的位置和标签不能由上传人删除或遮盖；项目、采购编号、上传人、服务器时间和来源值不能手工修改。收货确认单可以填写一条统一备注，每张照片也可以单独填写备注；存在照片备注时优先展示照片备注，否则使用统一备注。备注只能通过业务表单维护，由服务端重新生成水印，不能直接编辑水印图片。系统同时保存原图、水印图和两者的哈希关联，日常业务查看优先展示水印图。

提交前可以预览、删除、重拍和修改备注；修改备注后重新生成水印。草稿中删除或重拍淘汰的照片按临时文件规则清理，不作为正式业务证据保留。首次提交后，已提交照片的原图、来源、服务器时间、水印和备注全部锁定，不能删除、覆盖或替换；补充证据只能追加，并记录追加人、时间和原因。水印生成失败时不得提交收货确认。

乙方送货单为可选附件，不是收货提交必填项。送货单可以系统拍照或相册上传，服务端同样生成水印并单独分类为“乙方送货单”，但不能替代至少一张材料或卸货现场照片；已上传送货单应进入收货确认单 PDF 附页。

物资主管复核至少可以：

- 复核通过。
- 退回修改数量、补充照片或说明，并填写退回原因。

物资主管不能修改采购明细或替经办人重做收货确认。

物资主管复核通过前，经办人或受托人可以按退回意见修改最终数量和说明并重新提交，首次提交后已锁定的照片仍不能改写，只能追加。复核通过后、采购正式办结前，物资主管可以撤销本次复核；撤销必须填写原因并进行明确二次确认，不要求再次输入当前密码。撤销后收货确认退回可修改状态，原复核、撤销和 PDF 刷新历史全部保留。采购正式办结后，不再允许撤销复核、修改收货结果或追加照片。

物资主管复核通过后生成独立的《项目零星材料收货确认单》PDF，至少展示项目、采购编号、供应商、经办人、委托关系、各材料审批数量、合格实际收货数量、不合格数量、无偿附赠数量、差异、备注、物资主管复核结果、姓名和时间。全部正式水印照片和已上传的乙方送货单进入 PDF 图片附页。

### 6.8 发票与无票

采购的发票入口可以从已上传付款凭证的付款详情进入，但发票记录保存到共享发票账本，并通过金额分摊关系覆盖采购及其付款申请。

本期零星采购票据写入必须同时满足：当前最终收货已经由物资主管复核通过，且采购已存在至少一笔有效公司实际付款或已执行供应商余额抵扣。该资金事实是票据登记的启动条件，不是整张采购票据的金额上限；未填写付款单归属时，票据可在当前收货实际成本内一次覆盖尚未支付完的采购金额。填写付款单归属时，该付款单自身必须已经发生有效资金执行，且归属金额不得超过该付款单实际执行金额；付款单归属只用于追溯，不能再次增加采购覆盖金额。待财务主管复核的无票确认和票据异常先占用对应采购行、付款单、发票行和采购根单额度，只有有效发票分摊、已确认无票和已确认票据异常计入正式覆盖。

采购详情必须返回当前采购、版本和收货修订坐标下的发票记录与发票行、有效及已冲销分摊、无票确认、票据异常、证明文件、状态和复核/冲销历史，并保留可直接调用复核或冲销接口的业务标识。付款详情返回其中明确归属于该付款单的同类事实；列表只保留轻量汇总。任何付款标识与采购、版本或收货修订坐标不一致的票据事实都不得计入付款归属金额，读模型必须失败关闭并提示核对。

发票唯一身份由服务端生成，不接受客户端提交。存在完整发票代码和号码时，使用两者经 NFKC、Unicode 空白归一和大小写归一后的无歧义版本化结构计算 SHA-256；否则必须提供可识别票据编号并以其规范化值计算。身份字段禁止控制字符和不可见格式字符，并在规范化完成后按 Unicode 码点执行长度校验。发票类型、金额、项目和文件不参与身份键；命中既有身份后逐项核对发票事实，不一致时固定拒绝，避免后补可识别编号把同一张发票登记两次。

零星采购发票登记的精确权限为：当前采购经办人、有效物资主管、当前项目财务人员或有效财务主管。普通物资员不能代替其他采购经办人登记，普通财务人员不能凭其他项目或仅全局身份跨项目登记。无票和票据异常只能由当前采购根单及有效版本共同冻结的经办人发起，确认、退回、冲销以及发票分摊冲销只能由财务主管执行；所有写入均在事务内再次校验操作账号仍处于启用状态。
...
- 已经发生真实付款时，不得静默修改采购版本，也不得直接按正常发票覆盖；该采购进入票据异常待处理。

审批为有票的材料不能直接改走普通无票确认。尚未真实付款时，由有票变无票属于采购条件变化，必须创建新采购版本并重新审批。已经真实付款、供应商最终无法提供符合冻结条件的发票时，采购经办人发起“票据异常确认”，填写原因并上传证明，由财务主管确认后将对应金额转为已确认无票；不重复发起采购或付款审批。
...
```text
有效发票覆盖金额 + 已确认无票金额 = 本次采购实际成本
```

退款、转为供应商余额和因少货取消的未支付审批额度均不属于本次采购实际成本，不要求发票，也不能计入本次采购的发票或无票覆盖金额。

### 6.9 收货差异、退款与供应商余额

本次采购实际成本按最终合格收货数量和当前有效采购版本冻结单价计算：

```text
有票材料实际成本 = 合格实际收货数量 × 冻结含税单价
无票材料实际成本 = 合格实际收货数量 × 冻结无票单价
本次采购实际成本 = 所有材料实际成本之和
```

原采购审批数量、单价、金额和审批单永久保持不变。详情和收货确认单同时展示“采购审批金额、本次采购实际成本、差异金额”，不能用实际成本覆盖原审批金额。合格实际收货数量不得超过审批数量；超出部分只有明确登记为无偿附赠时才能留在本单，否则必须另行发起采购。

最终结算按以下规则处理：

- 公司实际付款与已执行供应商余额抵扣合计小于本次采购实际成本时，只继续支付或抵扣尚欠的实际应付差额。
- 公司实际付款与已执行供应商余额抵扣合计等于本次采购实际成本时，直接结清。
- 公司实际付款超过扣除供应商余额抵扣后的实际应付金额时，只有真实多付部分需要处理。
- 审批金额中尚未支付、且因少货不再需要支付的额度直接取消，不形成退款，也不形成供应商余额。

已复核的收货结果存在少货时，采购经办人发起差异处理；存在真实多付时，由采购经办人在发起时选择“拟全额退款”或“拟全额转供应商余额”。物资主管只确认合格实际收货数量、不合格数量和差异事实，不再经过项目经理审批。差异金额只能由系统按冻结单价计算，经办人和物资主管都不能手工填写金额，最终资金处理仍由财务岗位按本节规则完成。

真实多付差额必须整笔选择以下一种处理方式，不允许在同一次差异处理中拆分：

1. 供应商全额退款。
2. 全额转为同项目、同供应商的供应商余额。

供应商退款只有在财务人员登记实际到账并绑定银行入账回单或现金收款证明后才生效，不能用退款承诺、聊天记录或手工说明减少本次采购实际支出。退款生效后形成不可变退款分录。

转供应商余额不是新的银行付款，不生成虚假的付款凭证。财务主管必须在明确确认框中输入当前密码后确认转入；确认成功后形成不可变供应商余额分录，并将该金额从本次采购净实际支出中转出。供应商余额严格隔离在同一项目、同一供应商范围内，不能跨项目或跨供应商使用。

后续同项目、同供应商采购存在可用余额时，系统默认优先抵扣。财务主管可以减少抵扣或本次不抵扣，但必须填写原因并留下审计。抵扣金额在付款申请提交前确定，作为“供应商余额抵扣 + 公司实际付款”的付款组成与付款申请一起审批：

- 付款申请提交时预留供应商余额。
- 付款申请退回、驳回、撤回、作废或因版本变化失效时释放预留。
- 付款审批通过只形成抵扣授权，不等于已经执行抵扣。
- 财务主管在执行环节通过确认框和当前密码确认抵扣生效。
- 抵扣执行后生成不可变余额分录，不要求上传银行付款凭证；公司实际付款部分仍必须登记付款凭证。
...
当本次采购实际成本已经全部结清后，关联付款申请中未执行且不再需要的批准额度自动标记为“未执行额度已取消”。批准金额和审批历史保持不变，付款详情与 PDF 分别展示批准金额、实际付款、余额抵扣和取消额度。
...
```text
当前采购版本已审批
+ 最终收货确认已提交且物资主管复核仍然有效
+ 不存在待补货、待处理的不合格品或未完成的收货差异
+ 采购资金结算额 = 本次采购实际成本
+ 有效发票覆盖金额 + 已确认无票金额 = 本次采购实际成本
= 采购办结
```
...
- 不存在仍有效的待审批付款、待执行实际付款或未释放供应商余额预留。
- 不存在待到账退款、待确认供应商余额转入或待执行供应商余额抵扣。
- 不存在发票和无票重复覆盖。
- 不存在待处理的票据异常或版本变更。

最后一个条件满足后，后端立即自动办结，前端不能直接提交“办结”布尔值。办结同时锁定采购、收货、差异结算、供应商余额关联和票据覆盖结果；办结后不允许撤销、退回或更正。
...
“办理中”表示付款执行、收货复核、差异结算、供应商余额或发票/无票至少还有一个条件未完成。
...
付款申请的审批状态、实际付款状态、供应商余额抵扣状态和最终结清状态必须分别保存和展示。“已执行供应商余额抵扣”不能显示为银行已付款；只有实际付款记录和有效付款凭证成功登记后，才能增加已付金额。
...
收货确认不建立批次状态。多辆车、多次拍照或多张送货单都归入同一张最终收货确认单。
...
收货复核不是四级审批，但物资主管复核通过后必须生成独立的《项目零星材料收货确认单》PDF，作为最终数量、影像、差异和复核事实的存档文件。
...
收货确认单在物资主管复核通过后开放最新版下载；复核被撤销时，原最新版标记为已失效并停止作为正式下载入口，重新复核通过后生成新的最新版。采购办结后固定为最终版，不再刷新。
...
- 审批人显示姓名快照。
- 实际付款和付款凭证。
- 最终收货数量、照片原图与水印图关联、复核和撤销复核。
- 收货差异、退款、供应商余额形成和抵扣分录。
- 发票分摊和无票确认。
- PDF 生成任务、结果和替换关系。
...
- 最终审批完成。
- 实际付款和付款凭证登记成功。
- 收货确认提交、物资主管复核通过或撤销复核。
- 补充收货照片、退款到账、供应商余额转入或抵扣执行。
- 发票分摊或无票确认生效。
- 票据异常确认生效。
- 有效业务更正或反向分录生效。

付款审批单只有在实际付款记录和付款证明登记成功后，才能显示实际付款金额、付款日期和凭证摘要。单纯审批通过或执行供应商余额抵扣不能显示银行已付。付款审批单应分别显示批准金额、实际付款、供应商余额抵扣、退款、转出供应商余额和未执行取消额度，并根据真实付款累计显示“未付款、部分已付或已付款”。
...
5. 采购审批历程。
6. 关联付款申请和实际付款。
7. 最终收货数量、照片、送货单、委托和物资主管复核。
8. 收货差异、退款和供应商余额。
9. 发票/无票覆盖与票据异常。
10. 最新采购申请单和收货确认单 PDF。
11. 审计摘要和下一步动作。
...
主要列包括采购编号、项目、供应商、经办人、受托人、采购审批金额、实际采购成本、差异金额、付款状态、照片数量、收货状态和更新时间。不展示或创建收货批次。
...
- 付款草稿自动创建、付款申请提交和失效。
- 实际付款、凭证绑定、凭证补充和反向流水。
- 收货委托、正式照片上传、水印生成、确认、退回、物资主管复核和撤销复核。
- 收货差异发起与确认、未执行额度取消、供应商退款、余额形成、预留、释放和执行抵扣。
- 发票上传、分摊、解除分摊、无票确认和票据异常确认。
- 税率选项新增、停用和调整。
...
- 发票分摊占用与无票金额确认。
- 收货确认、最终数量、水印文件绑定和物资主管复核。
- 收货差异确认、未执行付款额度关闭和实际成本冻结。
- 供应商退款到账、退款凭证绑定和净支出更新。
- 供应商余额形成、预留、释放、执行抵扣和关联采购更新。
- 最后一个办结条件满足时的采购自动办结与全单锁定。
- 借款实际放款与借款台账增加。
...
- 同一采购有效付款申请的结算申请金额合计不得超过当前有效采购版本金额。
- 同一付款申请实际付款合计不得超过批准的公司实际付款金额。
- 同一采购实际付款合计不得超过当前有效采购版本金额。
- 一张采购单只能存在一张当前有效的最终收货确认单，不建立收货批次。
- 合格实际收货数量不能超过审批数量；无偿附赠数量和不合格数量不能计入实际采购成本。
- 本次采购实际成本只能由合格实际收货数量和冻结含税/无票单价计算。
- 财务直付收款方必须匹配冻结供应商。
- 经办人垫付报回收款人必须匹配采购经办人。
- 发票分摊和无票金额不能重复覆盖。
- 有效发票与已确认无票只能覆盖本次采购实际成本，不能覆盖退款、供应商余额转出或未支付取消额度。
- 发票累计分摊不能超过票面金额。
- 同一发票身份在零星采购和费用报销中必须引用同一发票记录，不能通过重复建档重新获得可分摊金额。
- 收货确认至少有一张成功生成水印的照片。
- 收货照片不要求定位；系统拍照和相册上传均可提交，但必须保存来源标签。
- 同一次真实多付差额只能全额退款或全额转供应商余额，不能拆分处理。
- 供应商退款未实际到账并绑定凭证前，不能减少本次采购净实际支出。
- 供应商余额只能在同项目、同供应商范围形成、预留和抵扣。
- 供应商余额抵扣必须先预留，失败单据必须释放；执行抵扣必须由财务主管使用当前密码确认。
- 采购资金结算额必须等于本次采购实际成本。
- 采购变更不得覆盖已发生的实际付款。
- 借款余额只因真实放款增加。
...
- 所有最终办结状态由后端事实计算；办结后不得修改或撤销采购、收货、差异和票据事实。
...
- 所有报价单、附件、收货照片、送货单、付款凭证、退款凭证、发票和 PDF 均进入私有对象存储。
...
- 水印生成失败：不得提交收货确认，原始上传可以保留为待处理文件。
- 供应商退款凭证绑定失败：不得确认退款到账，不得减少本次采购净实际支出。
- 供应商余额并发预留冲突：本次提交整体失败，不得部分占用余额。
- 供应商余额执行确认失败：不得生成抵扣分录，不得把余额抵扣显示为已执行。
...
- 银行转账与现金付款证明。
- 付款和收货两种先后顺序。
- 单张最终收货确认、不建立批次、委托确认和物资主管复核。
- 系统拍照与相册上传均可提交、不采集定位、来源标签固定、照片必填和水印失败阻断。
- 收货提交后的照片锁定、补充照片、退回重提、办结前撤销复核和办结后禁止更正。
- 不合格、无偿附赠、少货和实际采购成本的精确计算。
- 少付继续支付、未支付额度取消、真实多付全额退款或全额转供应商余额。
- 供应商余额的同项目同供应商隔离、优先抵扣、预留、释放、当前密码确认和并发冲突。
- 退款未到账或无有效凭证时不能减少采购净实际支出。
- 发票多对多分摊、无票确认、票据异常确认、重复占用和按实际采购成本全覆盖。
- 采购办结条件逐项缺失时均不能办结，最后条件满足后立即办结且不可更正。
- 收货确认单 PDF 的照片附页、复核撤销失效和重新复核刷新。
- 借款分次放款，只按真实放款增加余额。
- 同项目同员工 FIFO 冲销。
- 财务调整借款批次但不能改变总冲销金额。
- 报销提交时的余额预留和并发冲突。
...
- 采购、付款、收货和票据状态的组合展示。
- 付款草稿补齐后才允许提交。
- 收货委托、最终数量、水印照片、来源标签和可选送货单展示。
- 采购审批金额、实际采购成本、退款、供应商余额和未执行取消额度的区分展示。
- 收货退回、撤销复核、差异处理和票据异常的下一步动作。
- 费用与报销三个一等动作。
...
5. 现金付款且有发票。
6. 无发票并由财务主管确认。
7. 原定有票、真实付款后无法开票，完成票据异常确认。
8. 先收货后付款。
9. 先付款后收货，并委托他人使用相册照片确认收货。
10. 少货且尚未付满，只支付实际成本并取消未执行额度。
11. 少货且已经多付，供应商退款实际到账并上传银行或现金收款证明。
12. 少货且已经多付，整笔转供应商余额，并在后续同项目同供应商采购中优先抵扣。
13. 同单记录不合格材料、无偿附赠材料、多张系统拍照或相册照片及可选送货单。
14. 多笔借款按 FIFO 冲销，报销金额小于借款余额。
15. 报销金额大于借款余额，产生公司补付，再登记员工还款。
...
- 实际付款和付款证明。
- 单张最终收货确认、照片水印、可选送货单和物资主管复核。
- 实际采购成本、少货差异、退款和同项目同供应商余额。
- 发票分摊、无票确认、票据异常确认和自动办结。
- 采购申请单、付款审批单和收货确认单最新版 PDF。
...
- 付款凭证失败时不产生已付事实。
- 水印照片失败时不能完成收货。
- 系统拍照和相册上传均能生成固定来源水印，且不请求定位权限。
- 收货差异、退款和供应商余额不能改写原采购审批金额。
- 供应商退款未到账、余额预留未释放或抵扣未执行时不能自动办结。
- 发票与无票不能重复覆盖。
- 旧单据可以继续读取和下载既有资料。
- 新旧入口切换有明确回退方式。
- 十五个真实或等价场景全部通过。
...
- 收货确认单能够展示最终数量、水印照片、差异和物资主管复核事实。
- 实际采购成本、未执行额度、退款和供应商余额能够正确结算且不改写原审批事实。
- 付款与收货任意先后都能正确闭环。
- 财务直付和经办人垫付都不会造成重复报销。
- 采购变更不会覆盖历史审批和付款事实。
...
查看下载权限、完整发票系统、完整采购管理，以及本设计已明确的最终收货少货结算之外的真实付款后退货、换货和跨单异常更正，均需另行设计确认，不能由本设计自动推导。
