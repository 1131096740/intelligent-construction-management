...
6. 报告中没有未知表、缺失表、缺失主键、未分类记录、未登记文件绑定、未知/混合归属、孤儿文件、重复对象键、悬空外键或候选循环依赖。候选表上的启用拒绝删除触发器默认仍以 `DELETE_GUARD_TRIGGER` 阻断；只有 #301 登记的 `PaymentExecutionPayerAttestation_evidence_immutable` 和 `VerifiedBankTransactionObservation_evidence_immutable` 两个精确 FileObject 条件守卫可进入候选级证明，且必须同时满足：触发器名、表、函数名/所在 Schema 及触发器/函数完整定义 SHA-256 与候选内置契约精确相等；所有 FileObject 外键绑定已登记；每个候选不被契约列明的四类受保护证据任何一行引用；对象内容/全部版本快照完整；写冻结租约有效。inspect、串行化锁内重建、每个 FileObject 删除前与 postcheck 均重新验证这些条件；任何名称、表、列、函数、定义摘要、引用、文件绑定、对象快照或写冻结漂移都失败关闭。该专用证明不禁用、删除或绕过任何触发器。
