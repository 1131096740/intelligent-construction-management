...
2026-07-16 只读预检进一步确认：生产运行、seed 停用、COS、LibreOffice 和必需中文字体技术项已通过；当前只有 4 份合同、1 份历史接管草稿、0 结算、0 付款，附件归档只有历史接管类型，不能替代真实 UAT。生产还缺少 `budget_director`，会阻断合同/结算审批，必须由业务负责人指定真实人员。现有生产仍为 `c1fcd236...`/50 个迁移，不能在旧版本上签认 `eaaffee56...`/51 个迁移的候选。详细证据与材料清单见 `docs/progress/2026-07-16-real-business-uat-preflight.md`。
