...
> 当前结论：**窗口 A 技术发布已成功；全面业务 Go-Live 仍等待真实业务/财务签认。**
...
### 8.4 窗口 A 实际发布结果（2026-07-19）

- 用户精确批准证据 HEAD `4b5b6f0a7dbb0b3271b63d682b00967bd81e1452` 与运行候选 `74d5d2449ab9e4232f2625f2805c64b1686ff314`；运行树零差异后原子推送候选分支和 `main`。
- GitHub Actions `29683793910` 完整通过 Verify build 与 Deploy to server；发布前异机备份、M52–M58/M69、运行时切换和健康检查成功。
- 生产服务器 HEAD、`origin/main` 与批准 HEAD 均为 `4b5b6f0a…`；迁移为 `69|0|0`，M69 为 54 个引用、54 个触发器、5 个统一函数、0 个旧函数。
- 公网 Web/API 200，API/Nginx/PostgreSQL/Cron active，TLS 证书有效至 2026-09-29，发布后 error/alert 日志为空；备份监控 timer active、最近结果 success。
- 生产只读 transition preview 为 0 项/0 阻断，未执行 `--apply`；transition 审计、被 transition 终止的审批、付款申请和实付均为 0。
- 技术部署成功不替代真实业务签认：我方主体仍为 0 条，4 个合同版本税务事实未确认，真实历史合同、活跃合同、母版/附件和普通岗位权限 UAT 仍需完成。
