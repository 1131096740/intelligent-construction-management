...
test("统一资金办理工作台只读取服务端聚合并按视图和来源筛选", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "视图" }).click();
  await page.getByRole("listitem", { name: "待我办理", exact: true }).click();
  await expect.poll(() => requests).toContain("/api/funds-workbench?view=pending_action&source=expense_reimbursement");
  await page.getByRole("textbox", { name: "视图" }).click();
...
async function mockFundsSession(page: Page, requests: string[]) {
  ...
  await page.route("**/api/funds-workbench*", (route) => {
    ...
    return route.fulfill({ contentType: "application/json", body: JSON.stringify({
      ...
      viewCounts: { all: 2, pending_action: 0, in_progress: 0, pending_funds: 2, partial_payment: 0, pending_refund: 0, pending_evidence: 0, completed: 0 },
      ...
      items: [
        {
          ...
          status: "approved_pending_payment", statusLabel: "已批待付", pendingRefund: false, pendingEvidence: false, pendingMyAction: false, updatedAt: "2026-07-23T10:00:00.000Z"
        },
        {
          ...
          status: "approved_pending_payment", statusLabel: "已批待付", pendingRefund: false, pendingEvidence: false, pendingMyAction: false, updatedAt: "2026-07-23T09:00:00.000Z"
        }
      ]
    }) });
  });
