...
const coverage = {
  ...
  normalInvoiceCents: "60000",
  confirmedNoInvoiceCents: "0",
  confirmedExceptionCents: "0",
  effectiveCoveredCents: "60000",
...
const ledger = {
  available: true,
  currentCoordinates: {},
  invoices: [{ id: "invoice-1", fileId: "invoice-file-1", status: "active" }],
  allocations: [{ id: "allocation-1", amountCents: "60000", status: "active" }],
...
test("runs final receipt, watermark preview, review and read-only closure UI", async ({ page }) => {
  ...
  await expect(page.getByText("发票覆盖", { exact: true }).first()).toBeVisible();
