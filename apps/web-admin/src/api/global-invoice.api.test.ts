...
import { allocateGlobalInvoice, createGlobalInvoice, createRedGlobalInvoice, createReissueGlobalInvoice, voidGlobalInvoice } from "./global-invoice.api";
...
describe("global invoice API", () => {
  ...
  it("uses only append-oriented global invoice and allocation commands", async () => {
    ...
    expect(mockApiFetch).toHaveBeenNthCalledWith(3, "/global-invoices/invoice-1/void", expect.objectContaining({ method: "POST" }));
    expect(mockApiFetch).toHaveBeenNthCalledWith(4, "/global-invoices/red", expect.objectContaining({ method: "POST" }));
    expect(mockApiFetch).toHaveBeenNthCalledWith(5, "/global-invoices/reissue", expect.objectContaining({ method: "POST" }));
  });
});
