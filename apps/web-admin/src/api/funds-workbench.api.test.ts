...
describe("funds workbench API", () => {
  ...
  it("keeps the default read free of empty filters and encodes selected server views", async () => {
    mockApiFetch
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ view: "pending_refund", source: "expense_reimbursement", items: [], viewCounts: {}, sourceCounts: {} }), { status: 200 }));

    await fetchFundsWorkbench();
    await fetchFundsWorkbench({ view: "pending_refund", source: "expense_reimbursement" });

    expect(mockApiFetch).toHaveBeenNthCalledWith(1, "/funds-workbench");
    expect(mockApiFetch).toHaveBeenNthCalledWith(2, "/funds-workbench?view=pending_refund&source=expense_reimbursement");
  });
});
