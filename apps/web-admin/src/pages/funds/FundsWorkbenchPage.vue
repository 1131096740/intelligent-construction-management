...
const counts = ref<Record<FundsWorkbenchView, number>>({ all: 0, in_progress: 0, pending_funds: 0, completed: 0 });
...
const summary = computed(() => `审批中 ${counts.value.in_progress} 条 · 已批待付 ${counts.value.pending_funds} 条 · 已完成 ${counts.value.completed} 条`);
