...
if [ -z "$command_name" ]; then
  echo "用法：run-business-zeroing-cli.sh <inspect|execute|verify|sign|dynamic|preflight-dynamic|isolated-file-cleanup> [参数]" >&2
  exit 64
fi
shift

case "$command_name" in
  inspect|execute|verify|sign|dynamic|preflight-dynamic|isolated-file-cleanup) ;;
  *)
