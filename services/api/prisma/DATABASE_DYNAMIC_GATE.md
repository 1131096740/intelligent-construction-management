...
本入口把当前 64 条受 `RUN_*` 控制的数据库测试纳入一次性本机 PostgreSQL 16 runner。清单以
...
最终标准输出是一行机器可读 JSON 收据，固定登记候选 SHA、迁移基线、镜像 ID、9 组结果及 64 条测试覆盖。
...
| --- | ---: | --- |
| contract-draft-aggregate-concurrency.spec.ts | 5 | `RUN_CONTRACT_DRAFT_AGGREGATE_DATABASE` |
...
| contract-ended-application-purge.spec.ts | 4 | `RUN_CONTRACT_DRAFT_AGGREGATE_DATABASE` |
...
这 40 条已通过统一 runner 补齐一次性数据库命名、127.0.0.1 绑定、完整迁移、固定环境开关、失败清理与候选收据；执行失败仍会使动态数据库总门保持阻塞。
