...
不提供 `--group` 时仍按清单执行全部 14 组。CI 可以重复提供 `--group <清单组名称>`，只执行选中的组：
...
canonical manifest 当前登记 52 个文件、226 条 pending tests，全部已有本机 PostgreSQL 16 runner，`remaining=0`。其中 `remaining_dynamic_postgresql16` 覆盖 37 个文件、146 条用例。

#284 的 `participant_history_integrity` 子组使用独立数据库 `jiangkong_participant_history_integrity_test`，在同一文件内执行原有经营档案 17 条与参与公司历史完整性 10 条，共 27 条：
