...
function assertDedicatedLocalDatabase(databaseUrl) {
  ...
  if (!["postgresql:", "postgres:"].includes(parsed.protocol)) {
    throw new Error(
      "项目支出撤回并发验收只能连接 PostgreSQL 临时数据库"
    );
  }
...
function assertLocalDockerEndpoint(endpoint) {
  ...
  if (
    ...
    throw new Error(
      `项目支出撤回并发验收拒绝远程 Docker endpoint：${normalized}`
    );
  }
}
...
async function waitForPostgres(containerName, dockerCommand) {
  ...
  throw new Error(
    "项目支出撤回并发验收临时 PostgreSQL 16 在 30 秒内未就绪"
  );
}
...
function createProjectExpenseWithdrawalConcurrencyCleanup({
...
async function main() {
  ...
  const containerName =
    `jiangkong-project-expense-withdrawal-${suffix}`;
  const temporaryRoot = await mkdtemp(
    path.join(tmpdir(), "jiangkong-project-expense-withdrawal-")
  );
  ...
  try {
    ...
    await command(
      ...
      [
        path.join(
          root,
          "services/api/prisma/verify-project-expense-withdrawal-concurrency.cjs"
        )
      ],
      {
        cwd: root,
    ...
    console.log(
      `项目支出撤回 PostgreSQL 16 并发验收通过（${EXPECTED_MIGRATION_COUNT} 个迁移）`
    );
  } finally {
    try {
      await cleanup();
