...
const DATABASE_NAME = "jiangkong_pol108";
...
function fail(message) {
  throw new Error(`POL-108 PostgreSQL 16 动态验收失败：${message}`);
}
...
async function assertExactCleanCandidate(environment) {
  const expectedSha = environment.POL108_EXPECTED_SHA ?? environment.DATABASE_DYNAMIC_GATE_CANDIDATE_SHA;
  if (!expectedSha || !SHA_PATTERN.test(expectedSha)) {
    fail("缺少完整 40 位 POL108_EXPECTED_SHA");
  }
  const head = (await command("git", ["rev-parse", "HEAD"])).stdout.trim();
  if (head !== expectedSha) fail("POL108_EXPECTED_SHA 与当前 HEAD 不一致");
...
async function main() {
  ...
  const containerName = `jiangkong-pol108-${suffix}`;
  const temporaryRoot = await mkdtemp(path.join(tmpdir(), "jiangkong-pol108-"));
  ...
  const runtimeEnvironment = {
    ...
    RUN_POL108_OPERATING_PROJECTION_PG16: "1",
    SEED_PASSWORD: `Local@1-${randomUUID()}`
  };
  delete runtimeEnvironment.POL108_EXPECTED_SHA;
  ...
  const cleanup = createRunnerCleanup({
    ...
    onComplete: () => console.log(`POL-108 动态验收清理完成：${containerName}`)
  });
  ...
  try {
    ...
    await command(process.execPath, [
      jestCli,
      "--runInBand",
      "src/database/operating-projection-postgresql.spec.ts"
    ], {
