...
ALTER TABLE "ProjectExpenseRequest"
    ADD CONSTRAINT "ProjectExpenseRequest_expenseType_check" CHECK ("expenseType" IN ('sporadic_payment', 'loan_reserve', 'comprehensive_expense')),
    ADD CONSTRAINT "ProjectExpenseRequest_expenseSubtype_check" CHECK ("expenseSubtype" IN ('sporadic_material', 'sporadic_machinery', 'sporadic_labor', 'temporary_service', 'other_sporadic', 'employee_loan', 'owner_loan', 'project_reserve', 'travel', 'entertainment', 'reimbursement')),
    ADD CONSTRAINT "ProjectExpenseRequest_type_subtype_match_check" CHECK (
        ...
        OR ("expenseType" = 'comprehensive_expense' AND "expenseSubtype" IN ('travel', 'entertainment', 'reimbursement'))
    );
