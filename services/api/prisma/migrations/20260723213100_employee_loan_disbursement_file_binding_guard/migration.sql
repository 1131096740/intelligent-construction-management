-- 借款实际放款凭证是新的非独占 FileObject 事实；仅扩展统一绑定守卫。
BEGIN;

SELECT pg_advisory_xact_lock(190731, 13);
LOCK TABLE "EmployeeProjectLoanEntry" IN SHARE ROW EXCLUSIVE MODE;
...
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
  VALUES
    ...
    ('ProjectExpenseRequest','attachmentFileId',FALSE), ('ProjectExpenseExecution','voucherFileId',FALSE),
...
DROP TRIGGER IF EXISTS jg_efb_employee_project_loan_entry_voucher ON "EmployeeProjectLoanEntry";
CREATE TRIGGER jg_efb_employee_project_loan_entry_voucher
BEFORE INSERT OR UPDATE OF "voucherFileId" ON "EmployeeProjectLoanEntry"
FOR EACH ROW EXECUTE FUNCTION jg_enforce_exclusive_file_business_binding('voucherFileId', 'false');
