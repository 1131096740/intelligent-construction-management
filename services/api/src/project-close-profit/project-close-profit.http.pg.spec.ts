...
describePg("POL-109 project close real HTTP / PostgreSQL 16", () => {
  ...
  it("completes all seven stages with separated roles, temporary distribution, exact company conservation and immutable history", async () => {
    ...
    const executionReconfirmLines = afterExecutionProfit.participatingCompanies.map(
      (company: { id: string; companyEntityId: string }) => ({
        projectParticipatingCompanyId: company.id,
        finalShareCents: company.companyEntityId === executableLine.companyEntityId
          ? afterExecutionProfit.currentProfitConfirmation.finalProfitCents
          : "0"
      })
    );
    ...
    expect((await request(
      ...
      financeDirector,
      "POST",
      commandBody(afterExecutionDistribution.projection.fingerprint)
    )).status).toBe(201);
    const finalWorkbench = await workbench(generalManager);
    expect(finalWorkbench.stages.slice(4).map((stage: { status: string }) => stage.status)).toEqual([
      "completed",
      "completed",
      "completed"
    ]);
    expect(finalWorkbench.history.distributions).toHaveLength(3);
  });
});
