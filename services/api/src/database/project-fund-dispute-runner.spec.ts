...
const runner = readFileSync(
  resolve(__dirname, "../../prisma/run-pol280-project-fund-dispute-local.cjs"),
  "utf8"
);

describe("POL-280 disposable PostgreSQL 16 runner", () => {
  it("is exact-SHA, clean-worktree, local-only and never pulls an image", () => {
    expect(runner).toContain("LOCAL_PG16_DYNAMIC_GATE");
    expect(runner).toContain("POL280_EXPECTED_SHA");
  ...
  it("deploys twice, seeds only the disposable database and runs the gated public-seam suite", () => {
    ...
    expect(runner).toContain('RUN_POL280_PROJECT_FUND_DISPUTE_PG16: "1"');
    expect(runner).toContain("project-fund-dispute-postgresql.spec.ts");
