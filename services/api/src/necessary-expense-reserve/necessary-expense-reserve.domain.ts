...
import {
  ...
  type NecessaryExpenseReserveEntryKind,
  type NecessaryExpenseReserveFundHolderKind,
  type NecessaryExpenseReserveReasonKind,
  type NecessaryExpenseReserveStatus,
...
export interface NecessaryExpenseReserveIdentityInput {
  projectId: string;
  affiliateAssignmentId: string;
  fundHolderKind: NecessaryExpenseReserveFundHolderKind;
  fundHolderId: string;
  basisKind: string;
...
export function buildNecessaryExpenseReserveIdentity(
  input: NecessaryExpenseReserveIdentityInput
...
export interface NecessaryExpenseReserveDraftCommand {
  reserveId?: string;
  entryId?: string;
  projectId: string;
  businessCode: string;
  ...
  fundHolderKind: NecessaryExpenseReserveFundHolderKind;
  fundHolderId: string;
  reasonKind: NecessaryExpenseReserveReasonKind;
  title: string;
  basisKind: string;
  basisBusinessIdOrEvidenceSha256: string;
  basisSummary: string;
  entryKind: NecessaryExpenseReserveEntryKind;
...
export interface ValidatedNecessaryExpenseReserveDraft
  extends Omit<NecessaryExpenseReserveDraftCommand, "amountCents" | "evidenceLevel"> {
  amountCents: bigint;
  evidenceLevel: "A" | "B";
  occurredAtDate: Date;
}

export function assertNecessaryExpenseReserveDraft(
  input: NecessaryExpenseReserveDraftCommand
): ValidatedNecessaryExpenseReserveDraft {
  const projectId = requiredText(input.projectId, "项目不能为空");
  const businessCode = requiredText(input.businessCode, "必要准备业务编号不能为空");
  ...
  if (!NECESSARY_EXPENSE_RESERVE_REASON_KINDS.includes(input.reasonKind)) {
    throw new BadRequestException("必要准备原因类型不正确");
  }
  ...
  return {
    ...
    fundHolderId,
    title: requiredText(input.title, "必要准备标题不能为空"),
    ...
    basisSummary: requiredText(input.basisSummary, "依据摘要不能为空"),
    evidenceFileId: requiredText(input.evidenceFileId, "私有证据文件不能为空"),
    reason: requiredText(input.reason, "本次准备原因不能为空"),
    adjustsEntryId,
    replacementImpacts,
    amountCents,
...
export function buildNecessaryExpenseReserveFingerprint(
  input: ValidatedNecessaryExpenseReserveDraft
): string {
  return sha256Jcs({
...
export function assertNecessaryExpenseReserveTransition(input: {
  action: NecessaryExpenseReserveTransitionAction;
  status: NecessaryExpenseReserveStatus;
...
}): NecessaryExpenseReserveStatus {
  if (!SHA256.test(input.expectedFingerprint) || input.fingerprint !== input.expectedFingerprint) {
    throw new ConflictException("必要准备冻结指纹已变化，请刷新后重试");
  }
  const requiredRoleByAction: Record<NecessaryExpenseReserveTransitionAction, RoleKey> = {
    submit: input.actorRoles.includes("finance_director") ? "finance_director" : "finance_staff",
    attest: "project_manager",
    confirm: "finance_director",
    return: "finance_director"
  };
  if (!input.actorRoles.includes(requiredRoleByAction[input.action])) {
    throw new ForbiddenException("当前岗位不能执行该必要准备动作");
  }
  ...
  if (input.action === "attest") {
    ...
    if (input.preparedByUserId === input.actorUserId) {
      throw new ConflictException("准备人与项目经理见证人必须是不同自然人");
    }
  }
  if (input.action === "confirm") {
    if (input.status !== "attested" || !input.attestedByUserId) {
      throw new ConflictException("必要准备必须先由独立项目经理见证");
    }
  }
  ...
  return {
    ...
    confirm: "confirmed",
    return: "returned"
  }[input.action] as NecessaryExpenseReserveStatus;
}
