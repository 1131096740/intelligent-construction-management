...
import { CONTRACT_COMMERCIAL_ENTRY_DEFINITION, CONTRACT_PARTY_ENTRY_DEFINITION, CONTRACT_PAYMENT_STAGE_ENTRY_DEFINITION, CONTRACT_PAYMENT_TERMS_ENTRY_DEFINITION, resolveContractTemplateEntry, resolveContractBillEntries } from "./contract-business-entry-definition";
...
export const CONTRACT_COMMERCIAL_ENTRY_POLICY: BusinessEntryTransactionScenePolicy = {
  ...CONTRACT_BASIC_ENTRY_POLICY, sceneKey: "contract_commercial_terms",
  resolveDefinition: async () => CONTRACT_COMMERCIAL_ENTRY_DEFINITION
};

async function partyVersionContext(context: BusinessEntryTransactionResolverContext) {
  const party = await context.tx.contractPartySnapshot.findUnique({ where: { id: context.target.entityId } });
  if (!party) throw new BadRequestException("合同主体快照不存在，请刷新后重试");
  return { ...context, target: { ...context.target, entityType: "contract_version", entityId: party.contractVersionId } };
}

export const CONTRACT_PARTY_ENTRY_POLICY: BusinessEntryTransactionScenePolicy = {
  sceneKey: "contract_party", targetKind: "project_owned_entity", entityType: "contract_party_snapshot", action: "contract.submit",
  resolveOwnership: async (context) => CONTRACT_BASIC_ENTRY_POLICY.resolveOwnership(await partyVersionContext(context)),
  resolveAuthorization: async (context) => CONTRACT_BASIC_ENTRY_POLICY.resolveAuthorization(await partyVersionContext(context)),
  resolveDefinition: async () => CONTRACT_PARTY_ENTRY_DEFINITION
};

async function termsVersionContext(context: BusinessEntryTransactionResolverContext) {
  const terms = await context.tx.paymentTermsVersion.findUnique({ where: { id: context.target.entityId } });
  if (!terms) throw new BadRequestException("付款条款版本不存在，请刷新后重试");
  return { ...context, target: { ...context.target, entityType: "contract_version", entityId: terms.contractVersionId } };
}

export const CONTRACT_PAYMENT_TERMS_ENTRY_POLICY: BusinessEntryTransactionScenePolicy = {
  sceneKey: "contract_payment_terms", targetKind: "project_owned_entity", entityType: "payment_terms_version", action: "contract.submit",
  resolveOwnership: async (context) => CONTRACT_BASIC_ENTRY_POLICY.resolveOwnership(await termsVersionContext(context)),
  resolveAuthorization: async (context) => CONTRACT_BASIC_ENTRY_POLICY.resolveAuthorization(await termsVersionContext(context)),
  resolveDefinition: async () => CONTRACT_PAYMENT_TERMS_ENTRY_DEFINITION
};

async function stageVersionContext(context: BusinessEntryTransactionResolverContext) {
  const stage = await context.tx.paymentTermsStage.findUnique({ where: { id: context.target.entityId } });
  if (!stage) throw new BadRequestException("付款阶段不存在，请刷新后重试");
  return { ...context, target: { ...context.target, entityType: "payment_terms_version", entityId: stage.paymentTermsVersionId } };
}

export const CONTRACT_PAYMENT_STAGE_ENTRY_POLICY: BusinessEntryTransactionScenePolicy = {
  sceneKey: "contract_payment_stage", targetKind: "project_owned_entity", entityType: "payment_terms_stage", action: "contract.submit",
  resolveOwnership: async (context) => CONTRACT_PAYMENT_TERMS_ENTRY_POLICY.resolveOwnership(await stageVersionContext(context)),
  resolveAuthorization: async (context) => CONTRACT_PAYMENT_TERMS_ENTRY_POLICY.resolveAuthorization(await stageVersionContext(context)),
  resolveDefinition: async () => CONTRACT_PAYMENT_STAGE_ENTRY_DEFINITION
};
