...
- [~] Issue #17 rebase 集成候选：已基于 #16 合并主线保留当前任职/归档治理与纯净草稿删除边界；结束申请仅在精确 `versionId`、完整项目阅览权限下打开只读历史，所有办理动作/复制广告关闭，最终驳回与已提交放弃经共同草稿写入边界拒绝。结束保留预览按范围分页，保留/解除由合同部主管审计执行。动态 PG16 RED 发现预览错误假定 `ContractVersion -> Contract` Prisma 关系；现先查当前合同部主管可见的合同 ID，再以 `contractId in (...)` 限定终态版本，定向服务/控制器/读取模型 98 项通过。真实路由断言也已改为默认工作台仅显示 3 条非结束记录。M122 及动态门清单已先 RED 后同步为 122 migrations、56 项/30 文件（remaining 配置实际 33 项）；route/web/capability 清单为 452 routes、0 blocker。当前不继承 #16 动态或生产证据；待干净精确 SHA 的本机 PostgreSQL 16 全门和双轴复核。
