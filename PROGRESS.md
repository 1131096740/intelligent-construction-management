...
- 2026-07-08 (CodeX)：继续推进改造方案 P1「最近打开 5 个单据」：后台骨架新增最近打开业务单据条，记录合同、合同工作台、结算和付款详情路径，浏览器本地去重保留 5 条，便于试运行人员在台账与详情之间回跳；本轮不做完整页签缓存。验证：Web Vitest 26 套 / 205 例、Web typecheck、Web lint 通过。
...
- [~] Web 管理端：企业后台、合同/结算/付款/资料库/审计、登录鉴权、委托台账、写操作入口、真实首页逐单工作池、审批中心五视图、列表筛选器、详情动作元数据、统一动作状态面板、最近打开业务单据、资料库行内授权下载已完成；小程序和生产等价长链路验收仍待收口。
