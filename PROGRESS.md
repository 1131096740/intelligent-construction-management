...
- [x] P0 本地代码收口已完成：后端核心闭环、企业级合同工作台、历史合同接管、历史余额扣减、真实首页、业务选择器、常用单据、私有文件、PDF、审计、权限和 Web/API 首版部署均已有可验证实现。
- [ ] 仍不能宣称已成为真实项目唯一业务事实：DNS 正式域名解析、COS 桶策略人工验收、真实数据初始化、日志告警、合同母版签认、权限矩阵和业务 Go-Live 签字未完成。
...
- [~] 生产等价环境复验：环境变量、密钥、COS 配置、PostgreSQL 本机监听、HTTPS/IP 访问、安全响应头、时间同步、服务器健康检查已验证；域名 DNS、COS 私有桶控制台策略和云安全组仍需人工确认。
...
- [~] 运维验收：数据库备份恢复演练、Nginx 安全片段、运行健康检查在服务器实测并留证；附件备份、错误告警和 COS 控制台策略待现场确认。
...
| 项 | 状态 | 当前事实 | 剩余缺口 |
| --- | --- | --- | --- |
| P0-1 历史合同接管 | [x] | 后端接管模型、A/B/C 分级、复核/确认 API、草稿编辑保存、接管资料上传绑定、项目权限、审计、付款前硬拦截已完成；Web 最小表单已完成。 | Excel/OCR、小程序为后续增强，不阻断 P0。 |
...
| P0-5 生产验收 | [~] | 生产验收 Runbook、只读检查脚本、核心链路 UAT 脚本、最终闸门清单已形成；服务器 readiness、迁移状态、IP HTTPS、安全响应头、UFW、PostgreSQL 本机监听、时间同步、备份恢复演练已通过。 | 正式域名 DNS 指向错误，COS 桶私有策略、附件备份、错误告警、权限矩阵仍需现场验收。 |
...
- 2026-07-08 (CodeX)：接入生产服务器 `162.14.116.192` 做 P0-5 生产等价验收：`verify:production-readiness` 全 PASS，seed 用户和 refresh token 已停用，Prisma 27 个迁移为最新，API 重启后健康，IP HTTPS 与安全响应头通过，PostgreSQL 仅本机监听，UFW 仅开放 22/80/443，数据库备份恢复到临时库并校验关键表数量通过；修复运行健康脚本把 `journalctl -- No entries --` 误计为 warning 的问题。生产库 `ContractGeneratedDocument` 当前只有 1 条 `draft/failed`，没有可用于母版验收包的成功草稿。剩余：`jgzg.site` 当前解析到 `198.18.0.58`，需改到 `162.14.116.192`；COS 桶策略、附件备份、错误告警、权限矩阵、合同母版和 Go-Live 仍待现场确认。
...
- [~] Web 管理端：企业后台、合同/结算/付款/资料库/审计、登录鉴权、委托台账、写操作入口、真实首页逐单工作池、审批中心五视图、列表筛选器、详情动作元数据、资料库行内授权下载已完成；小程序和生产等价长链路验收仍待收口。
