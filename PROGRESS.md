...
| P0-3 真实使用体验 | [~] | 结算/付款业务选择器、真实首页工作台、逐单 `GET /me/work-items`、P0 浏览器冒烟 mock、审批中心五视图、详情页后端动作元数据、详情页审批历史时间线、合同/结算/付款列表过程字段、资料库行内授权下载、委托人员选择器同项目收窄已完成。 | 复杂筛选器、小程序、生产等价真实用户长链路验收。 |
