...
- [x] 在执行项目接管边界已重新验证：现有系统可支撑历史合同汇总期初余额接管，但要宣布成为真实项目唯一真相，还必须补齐项目级接管批次、历史结算和历史付款明细、结构化付款条款、多方复核、资料库授权下载和接管后核验。
...
- [ ] 在执行项目接管：导入预检已可用；仍需完成真实合同 A/B/C 分级、资料补录、复核、当前密码二次确认接管；若要宣布系统成为唯一真相，还需补项目级接管批次、历史期初有效结算、历史付款/已批待付/实付/预付款/质保金/扣回明细、接管资料授权下载和接管后核验。
...
| P0-2 历史余额控制 | [x] | 已确认接管历史余额进入付款容量硬扣减；付款预览和合同详情金额摘要已接入。 | 历史结算明细/分摊来源、Excel/OCR 不阻断汇总余额 P0，但历史结算和历史付款逐笔明细会阻断“唯一真相”口径。 |
...
- 2026-07-09 (CodeX)：确认并落地 Web Admin UI 治理基线：TDesign 作为唯一基础组件库，新增项目级 `--jg-*` token 命名、`check:ui` 自动检查、合同台账/合同详情/合同模板库三类样板和“同类 UI 第三次出现前必须抽象”的 agent 宪法；`.superpowers/` 工作流 scratch 已纳入忽略规则，避免 SDD 报告误入库。验证：Web 组件与样板 Vitest、Web typecheck、Web lint、Web build、`check:ui`、`git diff --check` 通过。
- 2026-07-09 (CodeX)：按用户要求再次派出多智能体深度验证“在执行项目接管成为现实唯一真相”方案，分别从后端、前端、财务控制、导入对账和安全审计复核；结论已融合进办公化合同/结算工作台方案：现有系统可支撑历史合同汇总期初余额接管，但不能直接宣称历史结算/付款逐笔账本完整；方案新增接管截止日、期初事实、A/B/C 分级、多方复核、不得编造历史事实、期初可付款来源、接管工作台步骤、资料库授权下载和最小落地顺序。验证：文档空白检查和 `git diff --check` 通过。
- 2026-07-09 (CodeX)：按用户补充确认修订办公化方案优先级：在线 Word/Excel 体验不是后续可选增强，而是合同部、预算部和结算人员高频工作入口，必须与账本模型补强双主线推进；方案新增第一阶段办公体验原型、合同部/预算结算人员试用验收、ONLYOFFICE 合同正文编辑 POC 和结算在线表格体验原型的前置任务，同时继续强调在线编辑器和表格组件不能成为业务事实来源。验证：文档空白检查和 `git diff --check` 通过。
- 2026-07-09 (CodeX)：按多智能体可行性研究结论修订办公化合同/结算工作台方案：将落地顺序调整为“先补账本模型，再接办公体验”，明确合同付款条款结构化录入和结算明细行账本为前置条件；ONLYOFFICE 仅做窄边界内网 POC，Collabora、Tabulator、SheetJS 降为后续专项验证；补充在线办公安全边界、回调验签、文件哈希、审批稿冻结和新会话第一批实施任务。验证：文档空白检查和 `git diff --check` 通过。
- 2026-07-09 (CodeX)：按用户确认的“前台像办公工具，后台像业务账本”和全项目中文业务化体验原则，完成办公化合同/结算工作台与效率提升方案：调研 ONLYOFFICE、Collabora、docxtemplater、PDF.js、Tabulator、SheetJS、ExcelJS、OpenProject、ERPNext、Odoo、NocoBase、JeecgBoot、Flowable 等开源项目，明确只融合成熟部件和产品形态，不引入低代码/BPMN/ERP 运行时；方案覆盖合同 Word 化编辑、结算 Excel 化录入、审批稿冻结、付款额度约束、全中文文案和新会话执行提示。验证：`git diff --check` 通过。
