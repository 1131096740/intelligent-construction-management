...
- [ ] 当前不进入本轮 P0：全量小程序、全项目铺开、历史合同逐份 AI/OCR 自动识别、历史合同重走审批、开票、考勤、人事、安全。
...
- [~] 文件/PDF/审计/安全：私有文件、本地/COS 存储、短时效下载、下载原因审计、审批单 PDF、水印、敏感二次确认、资料库治理筛选已覆盖主要链路；用户上传原始归档件水印暂不覆盖。
- [~] Web 管理端：企业后台、合同/结算/付款/资料库/审计、登录鉴权、委托台账范围/期限/代办标识、写操作入口、真实首页逐单工作池、审批中心五视图、列表筛选器、项目业务入口、跨项目老板经营总览、审批规则只读配置、字典/治理配置只读页、合同资金链时间轴、结算可付金额明细、付款实付入账覆盖关系、报销申请闭环、零星采购闭环、历史合同导入预检、详情动作元数据、统一动作状态面板、最近打开业务单据、资料库行内授权下载和下载原因已完成；小程序和生产等价长链路验收仍待收口。
