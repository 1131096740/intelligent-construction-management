...
- [~] Web 管理端：企业后台、合同/结算/付款/资料库/审计、登录鉴权、委托台账范围/期限/代办标识、写操作入口、真实首页逐单工作池、审批中心五视图、列表筛选器、项目业务入口、跨项目老板经营总览、审批规则只读配置、合同资金链时间轴、结算可付金额明细、付款实付入账覆盖关系、报销申请闭环、零星采购闭环、历史合同导入预检、详情动作元数据、统一动作状态面板、最近打开业务单据、资料库行内授权下载已完成；小程序和生产等价长链路验收仍待收口。
