...
- [~] Issue #14 已完成本地实现，待独立复审/合并：双方最终归档原件接受私有 PDF、DOCX、PNG/JPEG 扫描件，保留原始摘要并只校验文件完整性，不再要求与审批原件页数、字节或版式一致；合同员上传接口强制 `contract.archive.upload`，合同部主管语义确认不再重试密码且仍写审计，确认前可替换（旧最终件 superseded）而确认后冻结选定版本。正式合同文件每次 ticket 创建与实际读取均实时重验权限并记录下载审计；`ContractFormalFile` 绑定继续排除所有正式文件，草稿候选查询仅限 `abandoned` 原始草稿，`effective`/`superseded`（含历史生效版本）永久排除。未改 Schema、迁移、生产数据/COS 生命周期或合同/收款凭证水印。验证：API 聚焦 5 套件 470 tests、Web 聚焦 4 文件 169 tests、两端 typecheck/lint、`check:ui`、API build，以及 6 项 manifest（含 built Nest route matrix）均通过。
