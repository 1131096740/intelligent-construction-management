...
- 2026-07-08 (CodeX)：接入生产服务器 `162.14.116.192` 做 P0-5 生产等价验收：`verify:production-readiness` 全 PASS，seed 用户和 refresh token 已停用，Prisma 27 个迁移为最新，API 重启后健康，IP HTTPS 与安全响应头通过，PostgreSQL 仅本机监听，UFW 仅开放 22/80/443，数据库备份恢复到临时库并校验关键表数量通过；修复运行健康脚本把 `journalctl -- No entries --` 误计为 warning 的问题。剩余：`jgzg.site` 当前解析到 `198.18.0.58`，需改到 `162.14.116.192`；COS 桶策略、附件备份、错误告警、权限矩阵、合同母版和 Go-Live 仍待现场确认。
