...
- [~] 外围模块闭环：报销申请已按现有 `ProjectExpenseRequest` 非合同类付款申请链路完成一等入口、四级审批、审批单 PDF 下载、已批待付、出纳实付、凭证、财务入账、最终 PDF 归档和审计；零星采购完整闭环待下一步。
...
- [~] 文件/PDF/审计/安全：私有文件、本地/COS 存储、短时效下载、审批单 PDF、水印、敏感二次确认、资料库治理筛选已覆盖主要链路；用户上传原始归档件水印暂不覆盖。
- [~] Web 管理端：企业后台、合同/结算/付款/资料库/审计、登录鉴权、委托台账范围/期限/代办标识、写操作入口、真实首页逐单工作池、审批中心五视图、列表筛选器、项目业务入口、跨项目老板经营总览、审批规则只读配置、合同资金链时间轴、结算可付金额明细、付款实付入账覆盖关系、报销申请闭环、详情动作元数据、统一动作状态面板、最近打开业务单据、资料库行内授权下载已完成；小程序、零星采购和生产等价长链路验收仍待收口。
- [~] 小程序移动端：`apps/miniprogram` 原生微信小程序骨架已建立，具备移动待办列表、待办详情、拍照上传附件入口和 API 封装；真实 appid、微信登录绑定、移动审批动作和二次确认仍待后续确认。
