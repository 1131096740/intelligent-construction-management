...
- [~] Issue #17 rebase 集成候选：已基于 #16 合并主线保留当前任职/归档治理与纯净草稿删除边界；结束申请仅在精确 `versionId`、完整项目阅览权限下打开只读历史，所有办理动作/复制广告关闭，最终驳回与已提交放弃经共同草稿写入边界拒绝。结束保留预览按范围分页，保留/解除由合同部主管审计执行。动态 PG16 RED 发现预览错误假定 `ContractVersion -> Contract` Prisma 关系；现改为参数化数据库内联接按授权项目过滤版本，避免物化全部合同 ID，且无授权项目时直接返回空页。定向服务/控制器/读取模型 99 项通过；真实路由断言也已改为默认工作台仅显示 3 条非结束记录。动态门清单补入此前漏记的 file-binding 用例，统一为 M122、57 项/31 文件（remaining 33 项），并以自检防止计数漂移；route/web/capability 清单为 452 routes、0 blocker。当前不继承 #16 动态或生产证据；待干净精确 SHA 的本机 PostgreSQL 16 全门和双轴复核。
